...
client.on('ready', () => {
   console.log(`----------------`);
      console.log(`star Bot- Script By : MdAx77x `);
        console.log(`----------------`);
      console.log(`ON ${client.guilds.size} Servers ' Script By : MdAx77x  Codes ' `);
    console.log(`----------------`);
  console.log(`Logged in as ${client.user.tag}!`);
client.user.setGame(`#help-js | .MdAx77x 🚬#4461 | #تقديم `,"http://twitch.tv/S-F")
...
client.on('message', message => {//xR1 Codes
  ...
  if (message.content.startsWith(adminprefix + 'setg')) {
    client.user.setGame(argresult);
     ...
     if (message.content === (adminprefix + "leave")) {//xR1 Codes
    message.guild.leave();   //xR1 Codes
  } else  
  if (message.content.startsWith(adminprefix + 'setw')) {
  client.user.setActivity(argresult, {type:'WATCHING'});//xR1 Codes
      message.channel.send(`**✅   ${argresult}**`)//xR1 Codes
  } else
  ...
  if (message.content.startsWith(adminprefix + 'sets')) {
    client.user.setGame(argresult, "https://www.twitch.tv/zero");
      message.channel.send(`**✅**`)//xR1 Codes
  }
  if (message.content.startsWith(adminprefix + 'setname')) {
  client.user.setUsername(argresult).then
      message.channel.send(`Changing The Name To ..**${argresult}** `)
} else
  if (message.content.startsWith(adminprefix + 'setprefix')) {//xR1 Codes
  client.user.setPrefix(argresult).then
      message.channel.send(`Changing Prefix ..**${argresult}** `)//xR1 Codes
} else
if (message.content.startsWith(adminprefix + 'setavatar')) {//xR1 Codes
  client.user.setAvatar(argresult);
    message.channel.send(`Changing The Avatar To :**${argresult}** `);//xR1 Codes
}
});//xR1 Codes
...
client.on('messageReactionRemove', (reaction, user) => {
  ...
  .addField("⦁`All types of codes in xR1 Codes Server 💬`⦁",' ‎ ')
...
`);
  ...
  .addField("⦁`All types of codes in xR1 CodesServer 💬`⦁",' ‎ ')
...
client.on("message", message => {
        ...
        if(!message.channel.guild) return;
                if(message.author.bot) return;
        let channel = message.guild.channels.find("name", "✽-التقديمات")///MdAx77x is one  
            if(!channel) return message.reply("**لانشاء روم ✽-التقديمات !!setsubmissions من فضلك اكتب الامر**")
            if(channel) {
            message.channel.send( message.member + ', **:timer:**').then( (m) =>{
              m.edit( message.member + ', **اسمك الحقيقى بالكامل ✍**' )///MdAx77x is one  
              m.channel.awaitMessages( m1 => m1.author == message.author,{ maxMatches: 1, time: 60*1000 } ).then ( (m1) => {
                  m1 = m1.first();
                  var name = m1.content;
                  m1.delete();
                  m.edit(message.member + ', **:timer:**').then( (m) =>{
                      m.edit( message.member + ', **عندك كام سنة 🎓**' )///MdAx77x is one  
                      setTimeout(() => {
                        m.delete()///MdAx77x is one  
                      }, 10000);
                      m.channel.awaitMessages( m2 => m2.author == message.author,{ maxMatches: 1, time: 60*1000 } ).then ( (m2) => {
                          ...
                          message.channel.send( message.member + ', **:timer:**').then( (m) =>{
                            ...
                            m.channel.awaitMessages( m1 => m1.author == message.author,{ maxMatches: 1, time: 60*1000 } ).then ( (m3) => {
                                ...
                                m3.delete();
                                message.channel.send( message.member + ', **:timer:**').then( (m) =>{
                                  m.edit( message.member + ', **VarوCost هل تعرف الفرق بين  📑**' )///MdAx77x is one  
                                  setTimeout(() => {
                                    m.delete()
                                  }, 10000);
                                  m.channel.awaitMessages( m1 => m1.author == message.author,{ maxMatches: 1, time: 60*1000 } ).then ( (m4) => {
                                      m4 = m4.first();///MdAx77x is one  
                                      var ask2 = m4.content;
                                      m4.delete();///MdAx77x is one  
                                      message.channel.send( message.member + ', **:timer:**').then( (m) =>{
                                        m.edit( message.member + ', **لماذا يجب علينا ان نقبلك ؟ اعطنا كودالشخص مسويه ؟🤔**' )
                                        m.channel.awaitMessages( m1 => m1.author == message.author,{ maxMatches: 1, time: 60*1000 } ).then ( (m5) => {///MdAx77x is one  
                                            m5 = m5.first();
                                            var ask3 = m5.content;
                                            m5.delete();///MdAx77x is one  
                      m.edit(message.member + ', **....جارى جمع البيانات**').then( (mtime)=>{
                        setTimeout(() => {
                          let embed = new Discord.RichEmbed()///MdAx77x is one  
                        .setColor('RANDOM')
                        .setTitle(`**تقديم ادارة** [__**${message.guild.name}**__]`)
                        .addField('**`الاسم`**', `${name}` , true)
                        .addField('**`العمر`**', `${age}` , true)
                        .addField('**`هما هي لغه البرمجة الخاصه به ؟ `**',`${ask}`)///MdAx77x is one  
        ...
        client.on('message', message=>{///MdAx77x is one  
            if(message.content.startsWith("#روم1")) {
            if(!message.channel.guild) return;///MdAx77x is one  
                if(message.author.bot) return;
                if(!message.member.hasPermission('ADMINISTRATOR')) return message.reply("**تحتاج الى `MANAGE_CHANNELS`**");
                message.guild.createChannel("✽-التقديمات", "text").then(c =>{
                    c.overwritePermissions(message.guild.id, {///MdAx77x is one  
                        SEND_MESSAGES: false
 
                          })///MdAx77x is one  
                })
    message.channel.send("**✅ تم انشاء روم التقديمات بنجاح**")///MdAx77x is one  
            }
            })
 
client.on("message", msg => { //xR1 Codes
  if(msg.author.bot) return;
...
client.on('message',async message => {
  let mention = message.mentions.members.first();///MdAx77x is one  
  if(message.content.startsWith("#رفض")) {
  if(!message.channel.guild) return;///MdAx77x is one  
  let acRoom = message.guild.channels.find('name', 'القبول-الرفض');
  if(!acRoom) return message.reply("#روم2 من فضلك انشاء روم **القبول-الرفض** او اكتب الامر");
  if(!message.guild.member(message.author).hasPermission("MANAGE_ROLES")) return;
  if(!mention) return message.reply("منشن شخص");///MdAx77x is one  
  ...
  acRoom.send(`**${mention} تم رفضك للاسف**`)
         ...
         if(!message.channel.guild) return;
                ...
                message.guild.createChannel("القبول-الرفض", "text").then(c =>{
                    c.overwritePermissions(message.guild.id, {///MdAx77x is one  
                        SEND_MESSAGES: false///MdAx77x is one  
 
                          })
                })
    message.channel.send("**✅ تم انشاء روم القبول والرفض بنجاح**") ///MdAx77x is one  
            }
            })///MdAx77x is one
